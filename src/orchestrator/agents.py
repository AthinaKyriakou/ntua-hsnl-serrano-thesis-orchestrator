...
async def process_plans(plans):
    ...
    async for plan in plans:
        ...
        if(selected_orchestrator == SWARM or selected_orchestrator== K8s):
            ...
            db_rec = DatabaseRecord(requestUUID=plan.requestUUID, state=PENDING_STATE, resource=None, yamlSpec=plan.yamlSpec, appID=None, appName=None, timestamp=timestamp)
